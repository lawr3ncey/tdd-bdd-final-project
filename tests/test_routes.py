...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        response = self.client.put(
            f"{BASE_URL}/{original_id}", json=test_product.serialize()
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(f"{BASE_URL}/{original_id}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
        self.assertEqual(data["id"], original_id)
        self.assertEqual(data["description"], "updated description")
    ...
    def test_list_products_by_name(self):
        ...
        response = self.client.get(f"{BASE_URL}?name={query_name}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_list_products_by_category(self):
        ...
        count = sum(1 for p in products if p.category.name == category)
